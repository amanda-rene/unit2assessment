...
foods.forEach(function(elem, index, arr){
  if (this.elem = `carbs`, `protein`){
    return this.elem + (elem * 4)
  }
  else if (this.elem.key = `fat`){
    return this.elem + (elem * 9)
  }
})
...
const saleProducts = products.map(function(elem, index, arr) {
  if( elem === `price`) {
    return elem - (elem * .25)
  }
  return products
...
const blueProducts = saleProducts.filter(function(elem, index, arr){
...
const orderTotal = blueProducts.reduce(function(acc,elem, index, arr){
  return (acc += elem)
}, 0)
...
const ellensInfo = [...helensInfo, `Ellen`, `ellen@gmail.com`]
...
const ellensEmail = { ...ellensInfo, email: `ellen@gmail.com`}
