...
for ( let i = 0; i < classes.length; i ++){
    for (let key in classes){
        if (key.includes (`inPerson`, `homework`)){
          classes[key] = false
        }
    }
}
...
//CODE HERE
  ...
  const teachStay = teachTrick.bind(`stay`)
////////////////////PROBLEM 14////////////////////
/*
...
function dogIntro(treat, toy){
    return `${this.name} is a ${this.breed} that loves ${treat} ${toy}`
}
...
//CODE HERE
  let phone1 = new Phone(`brand`, `model`, 4, `color`, false )
  
  let phone2 = new Phone(`brand`, `model`, 4, `color`, false )
  
let phone3 = new Phone(`brand`, `model`, 4, `color`, false )
...
Phone.prototype.sell() = function sell(){
    if (this.sold === false){
        return true
    }
